...
import { Play, X, Plus, Trash2, Globe, Users, Lock, Video as VideoIcon } from "lucide-react";
import { createPost } from "../../api/posts";
import { useToast } from "../../context/ToastContext";
import { useLanguage } from "../../context/LanguageContext";

export default function VideoUpload({ onClose, onComplete }) {
    const { t } = useLanguage();
    const [titulo, setTitulo] = useState("");
    const [descripcion, setDescripcion] = useState("");
    const [categoria, setCategoria] = useState("Musical");
    ...
    const visibilityOptions = [
        { id: "publico", label: "Público", desc: "Todo el mundo", icon: Globe },
    ...
    const handleFileChange = (e) => {
        ...
        validVideos.forEach(file => {
            ...
            setMediaList(prev => [...prev, { url, file, tipo: "video" }]);
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (mediaList.length === 0) return error(t('select_video_required'));
        ...
        try {
            ...
            formData.append("tipo", "video"); // Metadata for video feed filtering
            ...
            success(t('video_viralized'));
            onComplete();
            onClose();
        } catch (err) {
            error(t('error_viralizing'));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-3xl relative max-h-[90vh] overflow-y-auto scrollbar-hide border-x border-t border-red-50 sm:border-b">
                <button onClick={onClose} className="absolute top-6 right-6 p-2 hover:bg-black hover:text-white rounded-full transition-all text-gray-400">
                    <X className="w-5 h-5" />
                </button>

                <form onSubmit={handleSubmit} className="space-y-6 text-center sm:text-left">
                    <div className="flex flex-col items-center sm:items-start space-y-1">
                        <div className="p-3 bg-black text-white rounded-2xl shadow-lg mb-2">
                             <VideoIcon className="w-6 h-6" />
                        </div>
                        <h3 className="text-xl sm:text-3xl font-black text-gray-900 tracking-tighter uppercase">{t('video_upload_title')}</h3>
                        <p className="text-[8px] sm:text-[10px] font-black text-red-600 uppercase tracking-widest leading-none">{t('video_upload_subtitle')}</p>
                    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide justify-center sm:justify-start">
                        <div className="flex gap-3 mx-auto sm:mx-0">
                            {mediaList.map((item, index) => (
                                <div key={index} className="relative w-32 h-32 rounded-2xl overflow-hidden border-2 border-red-50 group shadow-md shrink-0 bg-black flex items-center justify-center">
                                    <video src={item.url} className="w-full h-full object-cover opacity-60" muted />
                                    <Play className="absolute w-8 h-8 text-white opacity-40" />
                                    <button
                                        type="button"
                            ...
                            <button
                                ...
                                className="w-32 h-32 rounded-2xl bg-black/5 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300 hover:bg-black hover:text-white transition-all gap-1 shrink-0 group"
                            >
                                <Plus className="w-8 h-8 group-hover:scale-110 transition-transform" />
                                <span className="text-[8px] font-black uppercase">{t('add_video')}</span>
                            </button>
                        </div>
                    </div>

                    <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} multiple accept="video/*" />
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className={`w-full py-4 sm:py-6 rounded-2xl sm:rounded-3xl font-black uppercase tracking-[0.3em] text-[10px] sm:text-base shadow-2xl transition-all active:scale-[0.98] ${loading ? 'bg-gray-200 text-gray-400 animate-pulse' : 'bg-black text-white hover:bg-gray-900 hover:shadow-gray-200'}`}
                    >
                        {loading ? t('viralizing') : t('launch_video')}
                    </button>
                </form>
            </div>
        </div>
