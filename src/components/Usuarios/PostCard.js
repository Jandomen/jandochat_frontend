...
import { MessageCircle, Send, MoreHorizontal, Heart, Share2, Edit3, Trash2, Repeat, X as CloseIcon, User as UserIcon } from "lucide-react";
...
export default function PostCard({ post, id, onReact, onComment, onReply, onDelete, onEdit, onShare, onDeleteComment, onEditComment }) {
    ...
    return (
        <div id={id} className="bg-white rounded-xl border border-gray-200 mb-4 shadow-sm">
            {/* Header */}
            <div className="p-4 flex items-start justify-between bg-gradient-to-r from-red-600 to-red-500">
                ...
                {post.isShared && post.sharedFrom ? (
                    <div className="border border-gray-200 rounded-xl p-4 mb-4 bg-gray-50">
                        <div className="flex items-center gap-3 mb-3 cursor-pointer" onClick={() => navigate(`/usuarios/${post.sharedFrom.usuario?._id}`)}>
                            ...
                            />
                            <div>
                                <h5 className="text-sm font-medium text-gray-900 hover:text-red-600">{post.sharedFrom.usuario?.nombre}</h5>
                            </div>
                        </div>
            ...
            {(post.reacciones?.length > 0 || post.comentarios?.length > 0) && (
                <div className="px-4 py-3 flex items-center justify-between text-sm text-gray-500 border-t border-b border-gray-100 bg-gray-50">
                    <div className="flex items-center gap-2">
                        {post.reacciones?.length > 0 && (
                            <div className="flex items-center gap-1">
                                ...
                                <span className="cursor-pointer hover:text-red-600" onClick={() => post.reacciones.length > 0 && setShowWhoReacted(true)}>
                    ...
                    <div className="flex gap-4 text-sm">
                        <span className="cursor-pointer hover:text-red-600" onClick={() => setShowComments(!showComments)}>{post.comentarios?.length || 0} comentarios</span>
                    </div>
                </div>
            )}
                ...
                <div className="flex items-center gap-1 flex-1">
                    <div className="relative flex-1">
                        <button
                            ...
                            className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg transition-all text-sm font-medium ${miReaccionTipo ? 'text-red-600 bg-red-50' : 'text-gray-600 hover:bg-gray-100'}`}
                        >
                            {miReaccionTipo ? (
                                <>
                                ...
                                <>
                                    <Heart className={`w-5 h-5 ${showReactions ? 'fill-red-600 text-red-600' : ''}`} />
                                    <span>Me late</span>
                                </>
                            )}
                        </button>
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg transition-all text-sm font-medium ${showComments ? 'text-red-600 bg-red-50' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                ...
                <div className="px-4 pb-4 pt-2 bg-gray-50 border-t border-gray-200">
                    <div className="space-y-4 max-h-[400px] overflow-y-auto">
                        {post.comentarios?.map((c) => {
                            ...
                            return (
                                <div key={c._id} id={`comment-${c._id}`} className="flex gap-3">
                    ...
                    <form onSubmit={handleSubmitComment} className="relative mt-4">
                        ...
                        <div className="flex gap-2 items-center bg-white p-2 rounded-full border border-gray-200">
                            ...
                            />
                            <button type="submit" disabled={!comentario.trim()} className="p-2 bg-red-600 text-white rounded-full hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed">
